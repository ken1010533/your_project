...
#########################################################################################################
    ...
    def 更新營業狀態(self):
        # 統一轉換為8位數格式（補前導零）
        self.資料框架["統一編號"] = self.資料框架["統一編號"].astype(str).str.zfill(8)
        
        # 處理公司資料
        for 公司 in self.查詢結果.get("公司資料", []):
            統編 = str(公司.get("統編", "")).zfill(8)  # 確保8位數
            掩碼 = self.資料框架["統一編號"] == 統編
            狀態 = str(公司.get("營業狀態描述", "無資料"))
            
            if 掩碼.any():  # 確保有匹配項
                self.資料框架.loc[掩碼, "是否營業"] = 取得營業狀態(狀態)
        
        # 處理未匹配的資料（保留原邏輯）
        未查詢掩碼 = self.資料框架["是否營業"] == "尚未查詢"
        self.資料框架.loc[未查詢掩碼, "是否營業"] = "查無資料"
